...
@section('content')
    <h1>Payroll Schedule Created</h1>
    
    <p>Your payroll schedule has been created successfully.</p>
    ...
    <div style="background-color: #f9fafb; border-left: 4px solid #1a1a1a; padding: 16px; margin: 24px 0; border-radius: 4px;">
        ...
        @if($payrollSchedule->next_run_at)
        <p style="margin: 0; color: #4a4a4a;"><strong>Next Run:</strong> {{ $payrollSchedule->next_run_at->format('F d, Y \a\t g:i A') }}</p>
        @endif
    </div>
    
    <p>The schedule is now active and will process payroll according to the frequency you've set.</p>
