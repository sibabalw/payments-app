...
use App\Traits\ExponentialBackoffRetry;
...
class PaymentService
{
    ...
    public function processPaymentJob(PaymentJob $paymentJob): bool
    {
        ...
        // Log operation start
        $logContext = LogContext::logOperationStart('process_payment_job', LogContext::create(
            $correlationId,
            null,
            $paymentJob->id,
            'payment_process',
        ...
        return $idempotencyService->execute($idempotencyKey, function () use ($paymentJob, $correlationId, $operationId, $logContext) {
            // Use exponential backoff retry for transient failures
            return $this->retryWithBackoff(
                function () use ($paymentJob, $correlationId, $operationId, $logContext) {
                    return DB::transaction(function () use ($paymentJob, $correlationId, $operationId, $logContext) {
                        try {
                            ...
                            if ($availableBalance <= 0) {
                                $errorMessage = 'Cannot process payment: escrow balance is zero or negative. Available: '.number_format($availableBalance, 2);
                                ...
                                Log::warning('Payment rejected due to zero or negative escrow balance', [
                            ...
                            // Explicit check: available balance must be sufficient for payment amount
                            ...
                            DB::afterCommit(function () use ($correlationId, $paymentJob, $businessId, $operationId, $logContext) {
                                LogContext::logOperationEnd('process_payment_job', array_merge($logContext, [
                            ...
                            DB::afterCommit(function () use ($correlationId, $paymentJobId, $errorMessage, $trace, $businessIdForError, $operationId, $logContext) {
                                LogContext::logOperationEnd('process_payment_job', array_merge($logContext, [
                                    'correlation_id' => $correlationId,
                                    'business_id' => $businessIdForError,
                                    'job_id' => $paymentJobId,
                                    'operation_id' => $operationId,
                                ]), false, [
                                    'error' => $errorMessage,
                ...
                return DB::transaction(function () use ($payrollJob, $correlationId, $operationId, $logContext) {
                    try {
                        ...
                        if (in_array($payrollJob->status, ['succeeded', 'processing'])) {
                            ...
                            // Update status to processing first (required transition)
                            if ($payrollJob->status === 'pending') {
                                $payrollJob->updateStatus('processing');
                                // Reload to get updated status
                                $payrollJob->refresh();
                            }

                            // Skip real payment processing - just update status to succeeded
                            ...
                            // Move logging outside transaction to reduce lock duration
                            DB::afterCommit(function () use ($correlationId, $payrollJob, $businessId, $adjustments, $operationId, $logContext) {
                                LogContext::logOperationEnd('process_payroll_job', array_merge($logContext, [
                                    'correlation_id' => $correlationId,
                                    'business_id' => $businessId,
                                    'job_id' => $payrollJob->id,
                                    'operation_id' => $operationId,
                                ]), true, [
                            ...
                            // Log error after transaction commits
                            DB::afterCommit(function () use ($correlationId, $payrollJobId, $errorMessage, $trace, $businessIdForError, $operationId, $logContext) {
                                LogContext::logOperationEnd('process_payroll_job', array_merge($logContext, [
                                    'correlation_id' => $correlationId,
                                    'business_id' => $businessIdForError,
                                    'job_id' => $payrollJobId,
