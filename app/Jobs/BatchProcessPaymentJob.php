...
/**
 * Batch Process Payment Job
 *
 * Processes multiple payment jobs using true bulk operations for bank-grade performance.
 * Uses BulkPaymentProcessingService to process entire batches atomically with minimal lock contention.
 */
class BatchProcessPaymentJob implements ShouldQueue
{
    ...
    public function handle(BulkPaymentProcessingService $bulkProcessingService): void
    {
        ...
        $totalJobs = count($this->paymentJobIds);

        Log::info('Starting bulk payment processing', [
            'job_count' => $totalJobs,
            'job_ids' => array_slice($this->paymentJobIds, 0, 10), // Log first 10 for debugging
        ...
        $subBatches = array_chunk($this->paymentJobIds, $maxBatchSize);
        $totalProcessed = 0;
        $totalFailed = 0;
        ...
        try {
            foreach ($subBatches as $batchIndex => $subBatch) {
                ...
                $result = $bulkProcessingService->processPaymentJobsBulk($subBatch);
            ...
            Log::info('Bulk payment processing completed', [
    ...
    {
        Log::error('Batch payment job permanently failed', [
            'payment_job_ids' => $this->paymentJobIds,
